...
export default function FlashcardSetup() {
  ...
  const [numeroCard, setNumeroCard] = useState(10)
  ...
  async function generaFlashcard() {
    ...
    try {
      const res = await fetch('/api/generate-flashcard', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ testo: testo.substring(0, 8000) }) })
      const data = await res.json()
      if (data.flashcard) {
        sessionStorage.setItem('klass_flashcard', JSON.stringify(data.flashcard))
  ...
  return (
    <Layout>
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '40px 32px' }}>
        ...
        <h1 style={{ fontSize: 24, fontWeight: 500, color: '#18181B', margin: '0 0 6px', letterSpacing: -0.5 }}>Genera flashcard</h1>
        <p style={{ fontSize: 13, color: '#71717A', margin: '0 0 28px' }}>L&apos;AI crea carte fronte/retro per memorizzare concetti chiave</p>
        ...
        <button onClick={generaFlashcard} disabled={loading || !testo} style={{ width: '100%', background: '#18181B', color: 'white', border: 'none', padding: '13px', borderRadius: 10, fontSize: 14, fontWeight: 500, cursor: 'pointer', opacity: (loading || !testo) ? 0.6 : 1 }}>
